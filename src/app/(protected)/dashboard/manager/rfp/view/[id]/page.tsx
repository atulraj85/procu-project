      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <CardContent>
            {rfpData.organization ? (
              <div className="space-y-2">
                <p className="font-medium">{rfpData.organization.name}</p>
                <p className="text-sm text-gray-600">{rfpData.organization.legalName}</p>
                {rfpData.organization.gstin && (
                  <p className="text-sm">GSTIN: {rfpData.organization.gstin}</p>
                )}
                <p className="text-sm">{rfpData.organization.address}</p>
              </div>
            ) : (
              <p className="text-gray-500">No organization info</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {rfpData.createdBy ? (
              <div className="space-y-2">
                <p className="font-medium">{rfpData.createdBy.name}</p>
                <p className="text-sm text-gray-600">{rfpData.createdBy.email}</p>
                {rfpData.createdBy.mobile && (
                  <p className="text-sm">{rfpData.createdBy.mobile}</p>
                )}
                <Badge variant="secondary">{rfpData.createdBy.role}</Badge>
              </div>
            ) : (
              <p className="text-gray-500">No creator info</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-sm">{formatCurrency(rfpData.estimatedBudget)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm font-medium">Delivery Date:</span>
                <span className="text-sm">{new Date(rfpData.deliveryDate).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm font-medium">Quote Cutoff:</span>
                <span className="text-sm">{new Date(rfpData.quotationCutoffDate).toLocaleDateString()}</span>
              </div>
            </div>
          </CardContent>
        </Card>
