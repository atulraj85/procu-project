...
const sendMessage = async (content: string, threadId?: string, isNewThread = false) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Only add to current messages if it's for the active thread
      if (newMessage.threadId === activeThreadId) {
        setMessages(prev => [...prev, newMessage]);
      } else {
        // If it's a new thread, switch to it
        setActiveThreadId(newMessage.threadId);
      }
      
      // Refresh threads to update last message info
      await fetchThreads();
  ...
  try {
    ...
    // Create message with explicit new thread flag
    const response = await fetch('/api/conversations/messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        isNewThread: true, // Add this flag
      }),
    });
    ...
    if (response.ok) {
      ...
      // Clear the messages for the new thread (it will be fetched automatically)
      setMessages([]);
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <p className="text-md font-semibold text-gray-900 truncate">
                        {thread.first_message_content.substring(20) || 'New discussion'}
                      </p>
                    ...
                    <div className="flex flex-col items-end ml-2">
                      ...
                      {thread.message_count > 1 && (
                        <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full mt-1">
                  ...
                  {messages.map((message) => (
                    <div key={message.id} className="flex space-x-3">
                      ...
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${
                        message.senderType === 'USER' ? 'bg-blue-500' : 'bg-green-500'
                      }`}>
